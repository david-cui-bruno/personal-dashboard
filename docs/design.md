...
| `--bg-2` | `#fafaf8` | sidebar |
| `--ink` / `--ink-2` / `--ink-3` | `#1c1c1e` / `#6b6b70` / `#a3a3a9` | text primary/secondary/tertiary |
...
Dark theme + alternate accents defined as variable overrides (see mockup). Heatmap
cells use four accent-tinted levels over `--heat-0`.
...
- **Mobile** — bottom nav with **Today** and **Notes** only (#064); chart is a section
  on Today.

## chosen variants (from the comparison labs)

- Consistency chart: **option A** — minimal vertical, weekday columns on top, months
  down the left, newest at bottom.
